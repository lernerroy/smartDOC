  ...
  function (
    ...
    return BaseController.extend("airportprofile.controller.Charges", {
      ...
      _discardDraft: function () {
        ...
        var ID = dataModel.getProperty("/header/ID");
        var self = this;

        $.get({
          url: `${sUrl}(ID=${ID},IsActiveEntity=false)/SiblingEntity?sap-valid-from=date%270001-01-01%27`,
          success: function (data) {
            $.ajax({
              url: `${sUrl}(ID=${ID},IsActiveEntity=false)?sap-valid-from=date%270001-01-01%27`,
              method: "DELETE",
            })
              .fail(function (err) {
                if (err.status !== 404) {
